...
// ---------------------------------------------------------------------------
// Default behavior: hidden elements excluded. --include-hidden opts in.
// ---------------------------------------------------------------------------

#[test]
fn geometry_default_excludes_invisible_elements() {
    // Default: visibleOnly=true (JS-side filter). Mock returns only the visible div.
    let server = geometry_server("eval_result_geometry_visible_only.json");
    let port = server.port();
    let handle = std::thread::spawn(move || server.serve_one());

    // No extra flags — hidden elements must be excluded by default.
    let mut args = base_args(port);
    ...
    assert_eq!(
        elements.len(),
        1,
        "only the visible element should be returned by default"
    );
    assert_eq!(elements[0]["visible"], true);
...
#[test]
fn geometry_include_hidden_includes_invisible_elements() {
    // --include-hidden: visibleOnly=false. Mock returns both visible and invisible elements.
    ...
    assert_eq!(
        elements.len(),
        2,
        "both visible and invisible elements should be returned with --include-hidden"
    );
    assert_eq!(elements[0]["visible"], true);
