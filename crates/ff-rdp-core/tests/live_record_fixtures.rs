...
fn live_eval_stringify() {
    ...
    // Mirrors the IIFE that `eval --stringify` wraps around the user expression.
    let js = "(function() { try { return JSON.stringify(document.querySelectorAll('a')); } catch(e) { if (e instanceof TypeError && e.message.includes('circular')) return '{\"error\":\"circular reference detected\"}'; throw e; } })()";
    ...
    let (_imm, result) = record_eval(
        ...
        js,
        None,
        Some("eval_result_stringify.json"),
    );

    assert!(
        result["result"].is_string(),
        "eval --stringify should return a JSON string"
    );
}
...
fn live_dom_text_attrs() {
    ...
    // Mirrors the JS generated by dom.rs OutputMode::TextAttrs for a single element.
    let js = "(function() { \
        ...
        var els = document.querySelectorAll('a'); \
        if (els.length === 0) return null; \
        if (els.length === 1) return '__FF_RDP_JSON__' + JSON.stringify(textAttrs(els[0])); \
        return '__FF_RDP_JSON__' + JSON.stringify(Array.from(els, textAttrs)); \
    })()";
    ...
    let (_imm, result) = record_eval(
        ...
        js,
        None,
        Some("eval_result_dom_text_attrs.json"),
    );

    assert!(
        result["result"].is_string(),
        "dom --text-attrs should return a string (sentinel-prefixed JSON)"
    );
}
...
fn live_a11y_summary() {
    ...
    // Mirrors A11Y_SUMMARY_JS from a11y_summary.rs.
    let js = "(function() { \
    ...
    let (_imm, result) = record_eval(
        ...
        js,
        None,
        Some("eval_result_a11y_summary.json"),
    );
