...
mod tests {
    ...
    /// (`transport::max_frame_bytes()`) staying at its default. `transport`'s
    /// own tests used to shrink that cap to 1024 bytes, so this test took a
    /// `read` guard on `transport::FRAME_CAP_LOCK` to keep the two from
    /// interleaving.
    ///
    /// iter-196 removed the guard along with the lock: no test shrinks the
    /// process-global cap any more (they pass a cap explicitly to
    /// `recv_from_with_cap` and friends), and `transport::RaisedFrameCap`
    /// makes shrinking it panic. That closed the class — the read-guard
    /// convention had four other 15–20 KB readers in `actors::*` that never
    /// took one, so it was never actually protecting anything.
    #[test]
