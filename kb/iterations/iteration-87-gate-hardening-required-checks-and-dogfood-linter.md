...
### Theme A — `live-tests` becomes a required branch-protection check [3/3]

- [x] `tools/branch-protection.sh`: gh-cli script that queries the current
...
- [x] Documented in `CONTRIBUTING.md`: the exact `gh api` call to apply
      the protection rule, scoped to `iter-*` branches via the
      `enforce_admins` + `required_status_checks` payload.
- [x] Integration test `tools_branch_protection_asserts_required_live_tests`
...
### Theme B — `check-dogfood-script` FAILs by default on iter-* branches [4/4] [pre_fix_repro_test: live_check_dogfood_script_fails_without_ff_rdp_live_tests_on_iter_branch]

- [x] Detect current branch via `git rev-parse --abbrev-ref HEAD`. If it
      matches `iter-*` (or the `BRANCH_NAME` env var set by CI does),
      treat the dogfood gate as required.
- [x] When required AND `FF_RDP_LIVE_TESTS` is unset, exit non-zero with
      ...
      FF_RDP_LIVE_TESTS=1`.
- [x] On non-`iter-*` branches, skip behavior preserved (exit 0 with the
...
- [x] Rule 2 (`bool-flag-positional`): flags `ff-rdp <subcmd> --<bool-flag>
...
- [x] Rule 3 (`missing-set-euo-pipefail`): file must start (after
      shebang/comments) with `set -euo pipefail`. Unit test
      `unit_lint_dogfood_script_requires_set_euo_pipefail`.
- [x] Rule 4 (`missing-sentinel-pattern`): script must contain
...
### Theme D — `xtask check-pre-fix-repro` [4/4]

- [x] Plan-schema extension: per-theme `pre_fix_repro_test: <slug>`
      annotation parsed by `iteration_plan.rs`. Backward-compatible
      (themes without the annotation are ignored).
- [x] `check_pre_fix_repro.rs`: implements the four-step verification
...
### Theme E — fix iter-86's buggy dogfood-script assertions [3/3] [pre_fix_repro_test: lint_flags_iter86_assertions_before_fix]

- [x] Theme B (lcp_note): replace `grep -qi 'headless'` with anchored
...
- [x] tools_branch_protection_asserts_required_live_tests: script exits 0
      when `live-tests` is in `required_status_checks.contexts`; non-zero
      otherwise; mock payload fixture in `tools/tests/`.
- [x] live_check_dogfood_script_fails_without_ff_rdp_live_tests_on_iter_branch:
...
- [x] live_check_dogfood_script_skips_on_main_without_ff_rdp_live_tests:
      with HEAD on `main` and `FF_RDP_LIVE_TESTS` unset, exits 0 with the
      existing SKIP message.
- [x] unit_lint_dogfood_script_flags_unanchored_grep: fixture with the
      iter-86 Theme B grep returns lint error mentioning "anchored" or
      "false-positive".
- [x] unit_lint_dogfood_script_flags_boolean_flag_with_positional: fixture
      with `--jq-strict 'path'` returns lint error mentioning the boolean
      flag and missing `--jq`.
- [x] unit_lint_dogfood_script_requires_set_euo_pipefail: fixture without
...
- [x] dogfood_script_full_run_iter_87: the sibling `.dogfood.sh` exits 0
