...
- [x] Include the observed `committed_url` and `ready_state` in the
...
- [x] Preserve the old behaviour behind `--no-wait` for cases where
...
- [x] Add `--since <navigation-index|all>`:
...
- [x] Document the new event in `kb/reference/network.md` (or the
  in-tree help).
...
- [x] Surface the path used in `meta.fallback_method`:
...
- [x] Add a fixture page (or a mock-server response) with three
  ...
  right URL shapes.
- [x] Regression test: on a CSP-eval-free page, the result remains
...
- [x] All workspace `cargo test -q` continues to pass; no test in
