...
- [x] Statistical-timing test (1000 iterations) shows median timing for full-token comparison vs first-byte-mismatch comparison within 5% of each other.
- [x] `test_refstore_capped`: register 100 000 refs in a tight loop; assert HashMap len caps at `MAX_REFS`.
- [x] `test_nav_boundary_url_truncated`: emit a tabNavigated with a 1 MB URL; stored value is exactly 4096 chars.
- [x] `test_terminal_escape_sanitized`: eval throws an exception containing `\x1b[2J`; stderr output contains `?` not the raw byte.
- [x] `FF_RDP_TRACE_RAW=1 ff-rdp ...` prints the warning on first line of stderr.
- [x] Poisoned-mutex injection test: the daemon dispatcher continues running and emits a `tracing::error!` event.
- [x] Synthetic bulk-frame test: subsequent JSON frames parse correctly after the bulk skip.
- [x] `test_longstring_actor_capped`: `length = u64::MAX` returns typed error with zero allocation; theme E AC.
- [x] `test_wait_for_doc_complete_deadline_strict`: 100 dom-loading events at 10ms interval do not extend timeout beyond `timeout_ms + 100ms`.
