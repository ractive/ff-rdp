...
### A. Home view [0/4]
- [ ] `commands/home.rs`: `{bin, description, version, daemon: {running, pid, port},
      ...
      error)
- [ ] Text renderer: `bin: ~/.cargo/bin/ff-rdp` (home collapsed to `~`), one description line,
...
- [ ] Hints are state-dependent: no browser → `launch --headless`, `doctor`; browser but blank
...
- [ ] `ff-rdp --help` unchanged; `ff-rdp help` unchanged; only the bare invocation changes

### B. `install-hook` [0/4]
- [ ] `install-hook --claude [--project] [--dry-run] [--uninstall]`: adds a `SessionStart` entry
...
- [ ] Idempotent: re-running with the same resolved path is a no-op (exit 0, says so); a changed
...
- [ ] `--codex` and `--opencode` variants following the AXI §7 file locations
      ...
      inert
- [ ] Hook output is the Theme A text view with `--no-hints` off and the `page` block limited to
      headings + the first 15 interactive entries — this runs on every session, so it must stay
      small

### C. Skill from one source [0/2]
- [ ] `commands/skill_doc.rs` generates the static part of `SKILL.md` (command groups, the
...
- [ ] `cargo run -p xtask -- check-skill-drift` diffs committed vs generated and fails on
      mismatch; wired into `.github/workflows/ci.yml`

## Acceptance Criteria [0/8]

- [ ] `home_without_browser_exits_zero_and_names_launch` (unit, no daemon): `ff-rdp` → exit 0,
      `results.browser.reachable == false`, hints include `ff-rdp launch`
- [ ] `live_home_with_page_lists_tabs_and_refs`: after `navigate <fixture>`, bare `ff-rdp`
      JSON has `results.tabs[0].url == <fixture>` and `results.page.interactive[0].ref`
      accepted by `click --ref`
- [ ] `home_text_view_is_bounded` (unit, fixture page with 300 links): text output ≤ 80 lines
- [ ] `install_hook_is_idempotent` (unit, `HOME` redirected to a temp dir): two installs produce a
      byte-identical `settings.json`; second run reports no-op
- [ ] `install_hook_repairs_a_moved_binary_path` (unit): an existing entry with a stale absolute
      path is rewritten, other hooks untouched
- [ ] `install_hook_uninstall_removes_only_its_entry` (unit)
- [ ] `check_skill_drift_fails_on_stale_skill` (xtask unit): editing the committed `SKILL.md` by
...
- **Exit code of bare `ff-rdp`.** 0. Today it is 2 (clap "missing subcommand"). Anything that
  ...
  this repo (`grep -rn '\bff-rdp\s*$' kb tools crates` before merging, and note the result here).
