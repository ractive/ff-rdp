...
scope_note: >-
  ...
  request-attribution hazard; do not silently expand259 or weaken its ACs. Preserve originating
  Firefox connection identity through event delivery and release. The lazy watcher
  uses a separate connection; releases must return through that connection. Define
  final ownership separately for rendered snapshots and actor-inspecting consumers,
  and handle release-queue pressure without silent loss.
...
282/279 do not close daemon console actor lifetime. Before release implementation, specify an ownership table for main-connection RPC results, main watcher, lazy watcher, rendered cache and stream consumers. Key actor ownership and releases by originating connection. A rendered cache may retain no live actor lease; establish that contract explicitly rather than deriving actor retention from its entry capacity.

Cover recursive actor-bearing fields and current symbol behavior, plus queue-full recovery without silent release loss. Measure server actor existence on the originating connection for startup-tab and lazy-tabless cases, two consumers, duplicate and filtered delivery, disconnect and sustained pressure. Keep the original acceptance criteria unchanged.259 must provide independently reviewed and merged semantics adequate for release replies; a documented259 limitation alone does not satisfy this dependency.
