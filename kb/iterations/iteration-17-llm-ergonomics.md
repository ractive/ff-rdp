...
- [ ] **LLM-optimized help output** — add a top-level `--llm` or `llm-help` subcommand
...
- [ ] **`perf` summary mode** — add a `perf summary` subcommand that aggregates resource
...
- [ ] **Third-party domain analysis** — add `perf --type resource --group-by domain` or
...
- [ ] **`dom` count mode** — add `--count` flag to return only the number of matching
      elements instead of all their content. Useful for quick structural queries like
      "how many scripts/images/forms on this page?"

- [ ] **Navigation + wait combo** — add `navigate --wait-text` or `navigate --wait-selector`
...
- [ ] **`cookies` on consent-gated sites** — cookies returned empty on comparis.ch because
...
- [ ] **`cookies` output should include domain and path** — even when cookies are returned,
...
- [ ] At least 4 improvements are implemented
- [ ] `ff-rdp llm-help` (or equivalent) returns a complete, structured reference in one call
