...
## Part A: Commands That Should Bypass Daemon [0/4]
...
### A1: Make `cookies` use `connect_direct()` [0/1]
...
### A2: Make `storage` use `connect_direct()` [0/1]
...
### A3: Make `a11y` use `connect_direct()` [0/1]
...
### A4: Make `sources` use `connect_direct()` [0/1]
- [x] `sources` has the same JS eval fallback pattern as `a11y`. Direct connection would let the native actor path work correctly.

## Part B: Simplify Daemon Message Routing [0/3]

### B1: Remove watcher-interception guard for cookies/storage [0/1]
...
### B2: Document which commands use daemon vs direct [0/1]
...
### B3: Consider: should non-streaming `network` and `console` bypass daemon? [0/1]
...
## Part C: Security Hardening [0/3]

### C1: Add daemon port to error context [0/1]
...
### C2: Document the RPC writer limitation [0/1]
...
### C3: Validate daemon registry on read [0/1]
...
## Part D: Test Updates [0/2]

### D1: Update e2e tests for commands that now bypass daemon [0/1]
...
### D2: Add test for daemon registry validation [0/1]
