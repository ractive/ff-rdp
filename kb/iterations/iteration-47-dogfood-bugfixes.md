...
### 2. Fix `scroll` commands reporting stale viewport position [0/3]
...
### 3. Add `--format text` for `geometry`, `network` summary, and `dom tree` [0/3]
...
### 4. Fix `reload --wait-idle` reporting 0 requests [0/2]
...
### 5. Fix `responsive` reporting implausible negative `rect.y` values [0/2]
...
### 6. Hide irrelevant global flags from static-output commands [0/1]
