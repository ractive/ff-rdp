...
type: iteration
date: 2026-04-06
tags: [iteration, click, type, wait, interaction]
status: planned
...
- [ ] Implement `ff-rdp-cli/src/commands/click.rs` — `ff-rdp click <selector> [--tab ...]`
- [ ] Implement `ff-rdp-cli/src/commands/type_text.rs` — `ff-rdp type <selector> <text> [--tab ...] [--clear]`
- [ ] Implement `ff-rdp-cli/src/commands/wait.rs` — `ff-rdp wait [--selector <css>] [--text <text>] [--eval <js>] [--timeout <ms>] [--tab ...]`
- [ ] Click implementation via eval: `document.querySelector(sel).click()`
- [ ] Type implementation via eval: set `.value`, dispatch `input` and `change` events
- [ ] Wait implementation: poll with eval in a loop with configurable interval and timeout
- [ ] Handle element-not-found errors gracefully (exit 1 with clear message)
