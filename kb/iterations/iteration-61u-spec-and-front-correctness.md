...
- [x] `live_network_set_cookie_longstring`: page sets a 50 KB `Set-Cookie`; `network --detail --headers --jq '.requests[0].response.headers[]|select(.name=="set-cookie").value'` returns the full value, not an actor ref dump.
- [x] Deserialization round-trip test for `startedListeners` / `stoppedListeners` passes.
- [x] `live_cache_disable_via_target_config`: cache-disabled state is honored on a `Cache-Control: max-age=3600` resource across navigation.
