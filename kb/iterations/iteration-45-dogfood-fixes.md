...
- [x] Investigate why `--full-page` doesn't change capture behavior (likely the RDP `screenshot` actor call isn't receiving the full-page param, or the param name differs in this Firefox version)
- [x] Fix the implementation to capture the entire page content
- [x] Add an e2e test that verifies full-page screenshots are taller than viewport height
- [x] Record updated test fixture if needed
...
- [x] Add text formatter for `perf summary` (domain breakdown table)
- [x] Add text formatter for `a11y` tree output
- [x] Add text formatter for `responsive` breakpoint results
...
- [x] Wire `--limit` through to `a11y` tree output (limit depth or node count)
- [x] Wire `--limit` through to `sources` list output
...
- [x] Route fallback debug messages to stderr for `a11y`
- [x] Route fallback debug messages to stderr for `sources`
...
- [x] Add `allow_negative_numbers = true` to the scroll `by` subcommand in clap
- [x] Add a test with negative `--dy` and `--dx` values

### 6. Add `scroll --top` / `--bottom` shortcuts [0/2]

Currently scrolling to top/bottom requires `--dy=-99999` / `--dy=99999` workarounds.

- [x] Add `scroll top` and `scroll bottom` subcommands (or `--top`/`--bottom` flags on `scroll by`)
- [x] Update help text and llm-help
...
- [x] Investigate timing of watcher attachment vs reload; fix if possible, or document the limitation

## Acceptance Criteria

- [x] `screenshot --full-page` produces an image taller than viewport on a page with scroll content
- [x] `perf summary --format text`, `a11y --format text`, and `responsive --format text` produce tabular output
- [x] `a11y --limit 5` and `sources --limit 5` respect the limit
- [x] `a11y` and `sources` fallback messages go to stderr, not stdout
- [x] `scroll by --dy -500` works without equals syntax
- [x] `scroll top` and `scroll bottom` work
