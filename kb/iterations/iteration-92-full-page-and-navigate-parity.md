...
### Theme A — restore `fullPage:true` plumbing through WindowGlobalTarget [0/5] [pre_fix_repro_test: pre_fix_repro_screenshot_full_page_taller_than_viewport]

- [ ] Trace `full_page` from `Command::Screenshot { full_page, .. }`
...
- [ ] Forward `full_page` end-to-end to the `WindowGlobalTarget`
...
- [ ] Add a unit test in `screenshot_content.rs` (or alongside the
      ...
      legacy path.
- [ ] Land the pre-fix repro live test
      ...
      content.
- [ ] dogfood_script Theme A: capture both viewport and full-page
      PNGs of a tall fixture page; assert their md5 differs AND
      `identify -format '%h'` reports full-page height > viewport
      height.

### Theme B — navigate reject-stale-readyState + run/index parity [0/5] [pre_fix_repro_test: pre_fix_repro_navigate_second_call_waits_for_new_commit]

- [ ] In `crates/ff-rdp-cli/src/commands/navigate.rs`, audit the
...
- [ ] Add a navigation epoch/SHA: capture `document.location.href` +
...
- [ ] Diagnose why `run`/`index` time out where `navigate` succeeds.
...
- [ ] Land the pre-fix repro live test
      ...
      flake the assertion.
- [ ] dogfood_script Theme B: `navigate A`, `navigate B`, `run -e
...
- [ ] `unit_navigate_rejects_stale_ready_state`: feed the
