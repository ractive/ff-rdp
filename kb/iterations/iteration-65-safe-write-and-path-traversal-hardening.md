...
first_call_sites:
  - primitive: "ff_rdp_cli::util::safe_write"
    site: "crates/ff-rdp-cli/src/commands/screenshot.rs::resolve_output_path (writes through safe_write instead of fs::write)"
  - primitive: "ff_rdp_cli::util::safe_create"
    site: "crates/ff-rdp-cli/src/commands/auto_consent.rs::install (opens the XPI destination with O_NOFOLLOW|O_EXCL)"
dogfood_path: |
...
### A. Helper
- [ ] Add `crates/ff-rdp-cli/src/util/safe_io.rs` exporting `pub fn safe_write(path, bytes) -> Result<()>` and `pub fn safe_create(path) -> Result<File>`.
- [ ] Unix: use `OpenOptions::custom_flags(libc::O_NOFOLLOW)`. Windows: rely on `CreateFile`'s default behaviour (no symlink resolution by default) plus an explicit `is_symlink` precheck on the parent.
- [ ] Unit-test the helper against a fixture symlink → expect typed error `SafeIoError::SymlinkRefused`.

### B. Call-site migration
- [ ] `crates/ff-rdp-cli/src/commands/screenshot.rs:290-301` — replace `fs::write` with `safe_write`.
- [ ] `crates/ff-rdp-cli/src/commands/index.rs:696-731` — same; the page-map writer currently uses `write_atomic` which also follows symlinks at rename time.
- [ ] `crates/ff-rdp-cli/src/commands/install_skill.rs` (audit for `fs::write` / `fs::copy` follow-the-symlink calls).
- [ ] `crates/ff-rdp-cli/src/script/recorder.rs` (audit for writers that touch caller-supplied paths).
- [ ] `crates/ff-rdp-cli/src/commands/auto_consent.rs:33-62` — switch the XPI write to `safe_create` (closes finding F-9; complements iter-64).

### C. `--output-root` containment
- [ ] Add a `--output-root <dir>` flag on commands that take output paths (screenshot, index, record).
- [ ] When set, the helper canonicalizes the destination and rejects writes whose canonical form is not a descendant.
- [ ] Test the rejection path with a `../` traversal attempt.

## Acceptance Criteria [0/6]

- [ ] `safe_write_rejects_symlink`: writing through a pre-existing symlink returns `Err(SafeIoError::SymlinkRefused)`.
- [ ] `safe_write_succeeds_on_regular_file`: ordinary write path still works on a fresh file and on overwriting a regular file.
- [ ] `screenshot_safe_write_wired`: `commands/screenshot.rs` uses `safe_write` (no `fs::write` left in the module).
- [ ] `index_safe_write_wired`: `commands/index.rs::write_page_map` uses `safe_write`.
- [ ] `output_root_rejects_traversal`: with `--output-root /tmp/maps`, `--output ../etc/foo.json` exits non-zero with a typed error.
