...
285 passed / 0 failed. `passed + failed == executed`, so the record reconciles. This is the first
fully-green sweep in this line — 178's had one failure (`live_110`), which
[[iteration-191-stale-launch-record-recycled-pid-kill]] fixed and which passed here.

**What changed about the evidence, and it is not nothing:** [[iteration-188-live-sweep-cost-and-parallelism]]
landed, so the 276-test CLI suite now runs at `--test-threads=6`. Across the sweep the 1-minute
load average ranged 4.35 → 224.94, with 30 of 34 samples above 100 and 16 above 190. 192 predicted
exactly this — "if it lands, conditions 6 and 7 get their load experiment for free and honestly" —
...
|---|---|---|---|---|
| 1 | `live_160_..._ref_click_asserts_handler_effect` intermittent, cause unknown | it fails a sweep again | `ok` | Failed iters 168 and 171; passed 173, 178 and now 192. Three consecutive passes, still no cause. On the next failure read `meta.route` / `meta.daemon_fallback` (iter-172) before guessing |
| 2 | Firefox launch-timeout budget may need to scale with sweep load | a sweep reports `launch_timeout>0` | `launch_timeout=0` | One observation (iter-170), four clean runs since — now including one at load 224.94, which is the condition's own worst case and did not reproduce it. The count names the tests when it fires; until then there is nothing to size a budget change against |
| 3 | `vanished`/`launch_timeout` runtime paths unit-tested only | a sweep reports non-zero counts that do not reconcile with the log | `vanished=0 launch_timeout=0`; `285 passed + 0 failed == executed=285` | Third sweep in a row where **only the no-op branches ran**. The live-coverage gap is untouched. Forcing it still costs a full sweep plus a deliberately timed kill for a result the unit tests already predict — and note 192 got the reconciliation check itself to pass cleanly, which the 2026-08-23 nine-test verdict gap did not |
| 4 | An unprovoked port-6000 death | a port-6000 browser dies during a sweep on a machine nobody touched | 34 pid samples at 10 s, all `alive=1 listen=1`, across a run peaking at load 224.94 | Second clean negative, and a better one than 178's: the browser survived far heavier contention this time. Still one machine. If `vanished>0` ever appears with no operator action to blame, run the polling hunt against iteration 173's Theme A suspect list |
| 5 | `live_104_..._manifest_fetch_canonical` daemon-starved past 20 s | it fails again, **or** fails once at a 1-minute load average below ~30 | `ok`, across loads 4.35–224.94 | Passed, so the second arm was never exercised — a pass still cannot tell "starved" from "wedged". Note the shape of this negative though: it passed *at load 224.94*, which weakens the starvation hypothesis more than 178's 4.4–28.1 range did. Capture the daemon-side timing, not only the client `Timeout` envelope, when it fires |
| 6 | `live_145_..._click_element_not_found_unchanged` 21.9 s under `-j6` | **simplified**: it fails a sweep. The old second arm ("fails under `-j6` with its message captured") is now redundant — every sweep *is* a `-j6` run since iter-188 | `ok` at `--test-threads=6` | The textbook "one green run is not evidence a load-sensitive defect is fixed" row, except the load arm is now genuinely covered rather than merely asserted. Keep watching; do not manufacture load |
| 7 | `live_109_throttle_block::live_throttle_slow3g_slows_fetch` unattributed setup failure at load ~190 | **simplified**: it (or any test) fails during launch/daemon start with a captured message. The "under contended load" qualifier is now satisfied by default | `ok`, with 16 of 34 samples above load 190 and a peak of 224.94 | The original observation was at load ~190; this sweep spent half its samples above that and the test passed. Until a message is captured it remains indistinguishable from condition 2. Still no synthetic spinner — 177, 178 and 192 all declined to invent a reproduction, and iter-188 made that unnecessary |
| 8 | `live_137_daemon_mode_parity::live_137_consent_accept_via_daemon` fails under sweep load with `live_target_count: 0` while `target_count: 1` | it fails a sweep again | `ok` | Fired once (iter-186's sweep, run 2 of 2); passed 192's. **192 left its AC unticked**: a pass is not "a second observation confirming the poll-timing diagnosis", and it was not forked into its own plan either, so neither arm of that AC was satisfied. Carried unchanged. Next occurrence: compare `uptime_seconds` and `buffer_sizes` at poll time rather than re-confirm the flake |
