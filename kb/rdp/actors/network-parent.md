...
- **`unwatchResources(["network-event"])` destroys the block-list and the throttling config** (iter-164, confirmed on FF 153). The state does *not* live on `NetworkParentActor` — every method delegates to the `NetworkEventWatcher` resource, which owns the `NetworkObserver` that actually holds `#blockedURLs` (matched in `#shouldBlockChannel`, then `channel.cancel(NS_BINDING_ABORTED)` in `#createNetworkEvent`) and the throttler. Unwatching the resource destroys that observer; the next `watchResources` builds a fresh one with an **empty** block-list. Firefox reports nothing — `setBlockedUrls` succeeded, so the CLI's echo is truthful and the enforcement is simply gone.

  This is what made `throttle --block <pat>` a no-op through iter-163: `navigate` subscribes to `[document-event, network-event]` via `ResourceCommand` and unsubscribes on teardown, and the bus's ref-count is per CLI **process** — it cannot know another process already asked the shared daemon connection to watch `network-event`. The daemon now drops/strips such client frames (`classify_client_resource_teardown` in `daemon/server.rs`), the same treatment `unwatchTargets` has had since iter-137. Both are `oneway: true` in `devtools/shared/specs/watcher.js`, so dropping them leaves no client waiting.

  Reproduction of the underlying Firefox behaviour, if you ever need it again: a `--no-daemon` navigate (its own connection, its own watcher) leaves a daemon-set block-list intact; a daemon-path navigate clears it. In-page navigation (`location.href = …`) and `reload` do **not** clear it — only the `unwatchResources` frame does.
