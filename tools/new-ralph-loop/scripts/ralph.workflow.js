...
1) cd ${pf.repo_root} and sync: git fetch origin. FIRST check whether a branch for this iteration already exists (git branch --list '${pf.branch_prefix}-${it.n}/*' and git ls-remote --heads origin '${pf.branch_prefix}-${it.n}/*') — if one does, you are RESUMING a previous attempt that was killed: git switch onto it, do not delete it, do not create a second branch with a different suffix, and go to step 2. Otherwise: git switch main && git pull --ff-only origin main, then create ${pf.branch_prefix}-${it.n}/<short-description> from main. Note the working tree is shared; if git switch refuses because of local modifications, those are a previous attempt's uncommitted work — commit them onto the correct branch (see step 4) rather than stashing or discarding them.
...
4) Implement everything in the plan: code, tests, error handling. ${delegation}
   WIP CHECKPOINTS — commit early, then keep committing. The moment you have made ANY durable edit (the first file you create or modify), commit it: git add -A && git commit -m "wip(iter-${it.n}): <what you have so far>" && git push -u origin <branch>. Re-commit after each meaningful chunk. Do NOT wait for the implementation to be finished, and do NOT wait for gates to pass — a WIP commit is allowed to be broken, untested and half-done. Rationale: implement agents in this loop are killed mid-iteration routinely (four times in this project's history: iterations 140, 144, 146, 151), and an agent that dies holding only uncommitted edits leaves its successor to recognise and rescue loose work by luck. Ugly WIP commits cost nothing — they disappear inside the single merge commit — and they make every kill point recoverable.
   If you find UNCOMMITTED work already on the branch when you start, it is a previous attempt at THIS iteration that was killed. Do not discard it and do not start over: inspect it against the plan, commit it as a WIP checkpoint first, then continue from there.
...
1) cd ${pf.repo_root} && git switch ${impl.branch}. Then run the discipline checks and fix what they find:
   ...
   b) ${skillDir}/scripts/ac-fidelity-check.sh --plan ${it.plan_path} --branch ${impl.branch} --base main (hard gate). For each failure: implement the missing test/code so the AC matches the diff, untick the AC, or annotate it with [deferred — new plan: <path>] (the annotation must be the LAST thing on the AC) and create that follow-up plan. Do NOT reword an AC to get past the gate — that is the failure mode iter-154 exists to stop. A ticked AC naming a live_* test additionally needs [verified: <YYYY-MM-DD>, <measured result>], which means actually running it (FF_RDP_LIVE_TESTS=1 cargo test-live) and pasting the real numbers; if you did not run it, leave it unticked.
