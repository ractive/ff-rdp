...
PROMPT_IMPLEMENT="You are implementing iteration ${ITER_NUM} of this project. Steps: 1) Create a new branch for this iteration (e.g. iter-${ITER_NUM}/short-description) from main 2) Read the iteration plan from ${PLAN_PATH} 3) Implement everything in the plan (code, tests, error handling) using agents whenever possible 4) Ensure all docs, help texts, and project documentation are updated to reflect changes 5) Run the project quality gates (read CLAUDE.md for the specific commands) 6) /create-pr 7) Immediately after the PR exists, kick off the GitHub Copilot review by running: gh pr edit <PR-number> --add-reviewer @copilot — fire-and-forget, do NOT wait for the review to come back. This lets Copilot run in parallel while phase 2 starts. ${DONE_SENTINEL}"
